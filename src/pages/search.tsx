...
// 🪐 デザインカンプ通りの初期宇宙データ（APIからデータが来たらこれに合流する）
const MOCK_PRODUCTS = [
...
const CATEGORIES = ['すべて', '恒星・星', '惑星・衛星', 'ブラックホール', '銀河・星雲', 'その他'];
...
export const SearchPage = () => {
  ...
  // 📡 Goのバックエンドからリアルな出品データを取得してモックと結合する
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const formattedRealData = realData.map((p: any) => ({
            ...
            rating: 5.0,
            status: p.status,
            isReal: true // さっき出品したリアルデータと識別するフラグ
          }));
          
          // リアルデータを先頭にして、初期の豪華モックとガッチャンコ
  ...
  const filteredProducts = products
    ...
    .sort((a, b) => {
      ...
      return 0; // 'newest' は配列の順序（リアルが上）のまま
    });

  return (
    <div className="min-h-screen bg-[#030611] text-slate-100 font-mono p-6">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        
        {/* 📋 左側：絞り込みサイドバー */}
        <div className="lg:col-span-1 space-y-6 bg-[#070b1e]/60 border border-slate-800/60 rounded-xl p-5 backdrop-blur-md h-fit">
          <div className="flex justify-between items-center border-b border-slate-800 pb-3">
            <h2 className="text-sm font-bold text-slate-200 tracking-wider">絞り込み</h2>
            <button 
              onClick={() => { setSearchQuery(''); setSelectedCategory('すべて'); setSortOrder('newest'); }}
          ...
          <div className="space-y-2">
            <label className="text-[11px] text-slate-400 font-bold uppercase tracking-widest block">カテゴリ</label>
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-1.5 rounded text-xs transition-all flex justify-between items-center ${
                    selectedCategory === cat
                      ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 shadow-[0_0_8px_rgba(6,182,212,0.15)] font-bold'
                      : 'text-slate-400 hover:bg-slate-900/50 hover:text-slate-200 border border-transparent'
                  }`}
                >
                  <span>{cat === '恒星・星' ? '✨ ' + cat : cat === 'ブラックホール' ? '🕳️ ' + cat : '🪐 ' + cat}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* 💰 価格帯 (カンプ再現用プレースホルダー) */}
          <div className="space-y-2">
            <label className="text-[11px] text-slate-400 font-bold uppercase tracking-widest block">価格帯</label>
            <input type="range" min="0" max="1000000000" className="w-full accent-cyan-500 bg-slate-950 h-1 rounded" disabled />
          ...
          {/* 🔄 並び替え */}
          <div className="space-y-2">
            <label className="text-[11px] text-slate-400 font-bold uppercase tracking-widest block">並び替え</label>
            <select
              value={sortOrder}
              onChange={(e) => setSortOrder(e.target.value)}
              className="w-full bg-[#0c101f] border border-slate-800 rounded px-3 py-2 text-xs text-slate-300 focus:outline-none focus:border-cyan-500/50"
            >
